...
int feedLaneBonusScores[11][2] = {{BONUS_1K_ROW,  BONUS_1K_COL},
                                  {BONUS_2K_ROW,  BONUS_2K_COL},
                                  {BONUS_3K_ROW,  BONUS_3K_COL},
                                  {BONUS_4K_ROW,  BONUS_4K_COL},
                                  {BONUS_5K_ROW,  BONUS_5K_COL},
                                  {BONUS_6K_ROW,  BONUS_6K_COL},
                                  {BONUS_7K_ROW,  BONUS_7K_COL},
                                  {BONUS_8K_ROW,  BONUS_8K_COL},
                                  {BONUS_9K_ROW,  BONUS_9K_COL},
                                  {BONUS_10K_ROW, BONUS_10K_COL},
                                  {BONUS_20K_ROW, BONUS_20K_COL}};

int abLaneBonuses[7][2] = {{AB_1K_ROW,      AB_1K_COL},
                           {AB_2K_ROW,      AB_2K_COL},
                           {AB_3K_ROW,      AB_3K_COL},
                           {AB_4K_ROW,      AB_4K_COL},
                           {AB_5K_ROW,      AB_5K_COL},
                           {EXTRA_BALL_ROW, EXTRA_BALL_COL},
                           {SPECIAL_ROW,    SPECIAL_COL}};

int sinkholeBonuses[3][2] = {{BONUS_2X_TOP_ROW, BONUS_2X_TOP_COL},
                             {BONUS_3X_TOP_ROW, BONUS_3X_TOP_COL},
                             {BONUS_5X_TOP_ROW, BONUS_5X_TOP_COL}};

int feedLaneBonusMultipliers[4][2] = {{BONUS_2X_LOW_ROW, BONUS_2X_LOW_COL},
                                      {BONUS_3X_LOW_ROW, BONUS_3X_LOW_COL},
                                      {BONUS_5X_LOW_ROW, BONUS_5X_LOW_COL},
                                      {SHOOT_AGAIN_ROW,  SHOOT_AGAIN_COL}};
...
bool getRightFlipperFeadLane() {
    bool res = bally.getRedge(RIGHT_FLIPPER_FEED_LANE_ROW, RIGHT_FLIPPER_FEED_LANE_COL);
    if (res) {
...
bool getLeftFlipperFeadLane() {
    bool res = bally.getRedge(LEFT_FLIPPER_FEED_LANE_ROW, LEFT_FLIPPER_FEED_LANE_COL);
    if (res) {
        advanceFeedLaneBonus();
        scores[curPlayer] += 500;
        displayScores(scores, numPlayers);
...
bool getBottomBLane() {
    bool res = bally.getRedge(BOTTOM_B_LANE_ROW, BOTTOM_B_LANE_COL);
    if (res) {
        bLane = true;
        advanceABLaneBonus();
        bally.playSound(9);
...
bool getBottomALane() {
    bool res = bally.getRedge(BOTTOM_A_LANE_ROW, BOTTOM_A_LANE_COL);
    if (res) {
        aLane = true;
        advanceABLaneBonus();
        bally.playSound(9);
...
bool getTopBLane() {
    bool res = bally.getRedge(TOP_B_LANE_ROW, TOP_B_LANE_COL);
    if (res) {
        bLane = true;
        advanceABLaneBonus();
        bally.playSound(9);
...
bool getTopALane() {
    bool res = bally.getRedge(TOP_A_LANE_ROW, TOP_A_LANE_COL);
    if (res) {
        aLane = true;
        advanceABLaneBonus();
        bally.playSound(9);
...
bool getRightOutLane() {
    bool res = bally.getRedge(RIGHT_OUT_LANE_ROW, RIGHT_OUT_LANE_COL);
    if (res) {
        advanceFeedLaneBonus();
...
bool getLeftOutLane() {
    bool res = bally.getRedge(LEFT_OUT_LANE_ROW, LEFT_OUT_LANE_COL);
    if (res) {
        advanceFeedLaneBonus();
        scores[curPlayer] += 1000;
        displayScores(scores, numPlayers);
...
void loop() {
    ...
    while ((numPlayers < 4 && !(ballInPlay = getAnyPlayFieldSwitch())) || numPlayers == 0) {
        ...
        creditBallDisplay(ballNums[curPlayer], credits);
        for (int i = 0; i < 4; i++) {
            bool enable = (i == numPlayers - 1);
            bally.setLamp(canPlay[i][0], canPlay[i][1], enable);
        }
    }
